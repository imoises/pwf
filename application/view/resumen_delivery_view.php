...
<form  action="/ResumenDelivery/traerResumen" method="POST">
        ...
        if(!is_null($datos)){
            ...
            <thead>
              <tr class="orange-text text-darken-3">
                  <th>Pedidos realizados en el mes</th>
                  <th>Mi ganancia</th>
              </tr>
            </thead>
            <tbody>
              <tr>';
              if($datos->Pedidos != 0){
              echo'<td>'.$datos->Pedidos.' pedidos</td>';}
              else{
              echo'<td>Nigún pedido realizado</td>';
                }
                $delivery = ($datos->Dinero*3)/100;
                if($datos->Dinero != 0){
                echo'<td>$'.$delivery.'</td>';}
                else{ echo '<td>$0</td>';};
                echo'
