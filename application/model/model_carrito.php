...
class Model_Carrito extends Model{
    ...
    public function add($articulo)
    {
        ...
        //nuestro carro necesita siempre una cantidad y precio articulo
        if(!$articulo["cantidad"] || !$articulo["precio"])
        {
            throw new Exception("Error, el articulo debe tener cantidad y precio!", 1);
        }
        //nuestro carro necesita siempre una cantidad y precio articulo
        if(!is_numeric($articulo["cantidad"]) || !is_numeric($articulo["precio"]))
        {
            throw new Exception("Error, cantidad y precio deben ser números!", 1);
        }
    ...
    public function update_carrito()
    {
	    ...
	    //restamos un producto al carrito
    public function restar($articulo = array())
    {
        //primero comprobamos el articulo a añadir, si está vacío o no es un 
        //array lanzamos una excepción y cortamos la ejecución
        if(!is_array($articulo) || empty($articulo))
